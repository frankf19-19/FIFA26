...
- **版本號**:網頁標題列右側顯示 `build rXX`。每次我改版都會 +1,你一眼就能分辨手上的 `index.html` 是不是最新。改版時我也會告訴你新版號。
- **即時新聞**:頭條會優先抓 **ESPN 公開新聞**(免金鑰,標籤顯示「ESPN」、可點開原文),抓不到才用「賽果自動生成」的頭條墊底。
  - 誠實提醒:ESPN 新聞是**英文**,且免費端點未必每隊都有即時更新。要中文即時新聞需付費新聞 API。
  - 新聞由 `news_engine.py` 每 15 分鐘更新,前端每 60 秒自動抓取。
